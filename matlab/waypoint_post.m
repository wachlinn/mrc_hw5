...
% the bag file
bag = rosbag('~/mrc_hw5_data/waypoint.bag')
...
figure(1);
%Plot the X and Y location of your robot. Saveas images/waypoint_odom_xy.png
plot(x(ii),y(ii));
...
figure(2);
%Plot of the heading in degrees vs. time. Saveas images/waypoint_odom_yaw.png
plot(t(ii),yaw(ii));
...
figure(3);
%Plot of forward velocity vs. time. Saveas images/waypoint_odom_u.png
plot(t(ii),u(ii));
...
figure(4);
%Quiver plot showing the location, yaw and speed of the robot.  Saveas images/waypoint_odom_quiver.png
quiver(x(ii),y(ii),u(ii),v(ii));
title('Quiver Plot of Turtlebot waypointstick Test');
xlabel('X [m]'); ylabel('Y [m]');
